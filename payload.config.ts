...
        { name: 'slug', label: 'Слаг', type: 'text', unique: true, required: true, admin: { position: 'sidebar' } },
        { name: 'tags', label: 'Теги', type: 'relationship', relationTo: 'tags', hasMany: true, admin: { position: 'sidebar' } },
        // НОВЫЕ ПОЛЯ: date и author (логично для блога)
        { name: 'date', label: 'Дата публикации', type: 'date', required: true, admin: { position: 'sidebar', date: { pickerAppearance: 'dayAndTime' } } },
        { name: 'author', label: 'Автор (опционально)', type: 'relationship', relationTo: 'users', admin: { position: 'sidebar' } },
        // ЗАМЕНА: Использование caseBlocks для секций
...
        ]},
        { name: 'highlightedCases', label: 'Кейсы для секции "Наши кейсы" (полная настройка)', type: 'array', minRows: 1, fields: [
...
          { name: 'themes', label: 'Темы', type: 'array', fields: [{ name: 'theme', label: 'Тема', type: 'text' }] },
...
        { name: 'posts', label: 'Список новостных постов', type: 'array', minRows: 1, fields: [
...
          { name: 'image', label: 'Изображение для превью', type: 'upload', relationTo: 'media', required: true },
          { name: 'date', label: 'Дата публикации', type: 'date', required: true, admin: { date: { pickerAppearance: 'dayAndTime' } } },
          { name: 'views', label: 'Просмотры', type: 'number', defaultValue: 85 },
          { name: 'themes', label: 'Темы', type: 'array', fields: [{ name: 'theme', label: 'Тема', type: 'text' }] },
        ]},
      ],
